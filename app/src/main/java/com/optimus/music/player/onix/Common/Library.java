...
public class Library {
    ...
    private static final ArrayList<SongListChangeListener> SONG_LIST_CHANGE_LISTENERS = new ArrayList<>();
    private static final ArrayList<AlbumListChangeListener> ALBUM_CHANGE_LISTENERS = new ArrayList<>();
    private static final ArrayList<GenreListChangeListener> GENRE_CHANGE_LISTENERS = new ArrayList<>();
 ...
 */
    ...
    public interface SongListChangeListener{
        void onSongRemoved(Song removed, int index);
        void onSongAdded(Song added, int index);
        void onSongModified(Song modified, int index);

    }


    public interface AlbumListChangeListener{
        void onAlbumRemoved(Album removed, int index);
        void onAlbumAdded(Album added, int index);
    }

    public interface GenreListChangeListener{
    ...
    /**
     * In certain cases like in , editing the
...
* */
    ...
    public static void addSongListener(SongListChangeListener listener){
        if (!SONG_LIST_CHANGE_LISTENERS.contains(listener)) SONG_LIST_CHANGE_LISTENERS.add(listener);
    }



    public static void removeSongListener(SongListChangeListener listener){
        if (SONG_LIST_CHANGE_LISTENERS.contains(listener)) SONG_LIST_CHANGE_LISTENERS.remove(listener);
    }

    private static void notifySongRemoved(Song removed, int index) {
        for (SongListChangeListener l : SONG_LIST_CHANGE_LISTENERS) {
            l.onSongRemoved(removed, index);
        }
    }

    private static void notifySongAdded(Song added){
        int index = songLib.indexOf(added);
        for (SongListChangeListener l : SONG_LIST_CHANGE_LISTENERS) {
            l.onSongAdded(added, index);
        }
    }

    private static void notifySongModified(Song modified){
        int index = songLib.indexOf(modified);
        for (SongListChangeListener l : SONG_LIST_CHANGE_LISTENERS) {
            l.onSongModified(modified, index);
        }
    }



    public static void addAlbumListener(AlbumListChangeListener listener){
        if (!ALBUM_CHANGE_LISTENERS.contains(listener)) ALBUM_CHANGE_LISTENERS.add(listener);
    }


    public static void removeAlbumListener(AlbumListChangeListener listener){
        if (ALBUM_CHANGE_LISTENERS.contains(listener)) ALBUM_CHANGE_LISTENERS.remove(listener);
    }

    private static void notifyAlbumRemoved(Album removed, int index) {
        for (AlbumListChangeListener l : ALBUM_CHANGE_LISTENERS) {
            l.onAlbumRemoved(removed, index);
        }
    }

    private static void notifyAlbumAdded(Album added){
        int index = albumLib.indexOf(added);
        for (AlbumListChangeListener l : ALBUM_CHANGE_LISTENERS) {
            l.onAlbumAdded(added, index);
        }
    }
    ...
    public static void scanForSearch (final Activity activity){
        if (hasRWPermission(activity)) {
            resetAll();
            setSongLib(scanSongs(activity));
            setArtistLib(scanArtists(activity));
    ...
    public static void sort(Context context) {
        ...
        if(Prefs.getSongSortOrder(context)==0) {
            Collections.sort(songLib);
        }
        if(Prefs.getAlbumSortOrder(context)==0) {
    ...
    public static void addPlaylistEntries(final View view, final Playlist playlist, final ArrayList<Song> songs){
        ...
        if (duplicateCount > 0){
            ...
            Toast.makeText(context, context
                    .getString(R.string.confirm_add_songs, newEntries.size(), playlist.playlistName), Toast.LENGTH_LONG)
                    .show();
    ...
    public static void deleteEntireFolder(final Context context,
        ...
        AlertDialog dialog = new AlertDialog.Builder(context)
                ...
                .setPositiveButton("Delete", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        deleteFolderItems(context, songsInFolder, folder, adapter);
                    }
                })
                .setNegativeButton(R.string.action_cancel, null)
                .show();
